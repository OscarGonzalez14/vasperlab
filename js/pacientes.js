  ...
  if(nombrePaciente != ""){
    ...
    $.ajax({
      ...
      success:function(data){
        console.log(data)
        Swal.fire('Se ha registrado un nuevo paciente','','success')     
      }
    }); 

    setTimeout ("explode();", 2000);
  }else{
    $('#data_pacientes_reg').DataTable().ajax.reload();
...
function limpiar_modal_paciente(){
 let items = document.getElementsByClassName('limpiar');
 console.log(items)
...
function eliminar_paciente(id_paciente){
  ...
  $.ajax({
    ...
    success:function(data)
    {
      console.log(data);
      if(data=="ok"){
        setTimeout ("Swal.fire('El paciente ha sido eliminado','','success')", 100);
      }else if(data=="existe"){
        setTimeout ("Swal.fire('El paciente posee una orden','','error')", 100);
      }           //alert(data);
      $('#data_pacientes_reg').DataTable().ajax.reload();
    }
  });
