...
class Ordenes extends Conectar{
    ...
    $sql= "SELECT d.numero_orden,p.nombre,p.cod_emp,d.fecha,d.empresa,p.id_paciente,d.estado from pacientes_o as p inner join detalle_orden as d on d.id_paciente=p.id_paciente where d.estado='0';";
...
/////////////////////LISTAR SOLICITUDES EN PROCESO
  public function get_solicitudes_proceso(){
    $conectar= parent::conexion();         
    $sql= "SELECT d.numero_orden,p.nombre,p.cod_emp,d.fecha,d.empresa,p.id_paciente,d.estado from pacientes_o as p inner join detalle_orden as d on d.id_paciente=p.id_paciente where d.estado='1';";
    $sql=$conectar->prepare($sql);
    $sql->execute();
    return $resultado= $sql->fetchAll(PDO::FETCH_ASSOC);
}
...
//////////////DATOS GENERALES DEL PACIENTE EN DETALLES DE SOLICITUD
    ...
    $conectar= parent::conexion();         
    $sql= "SELECT p.nombre,p.empresa,d.numero_orden from pacientes_o as p inner join detalle_orden as d on p.id_paciente=d.id_paciente WHERE p.id_paciente=? and d.numero_orden=?;";
    $sql=$conectar->prepare($sql);
...
public function get_detalle_solicitudes($id_paciente,$numero_orden){
  ...
  $sql='SELECT examen,fecha from detalle_item_orden where id_paciente=? and numero_orden=?';
  $sql=$conectar->prepare($sql);            
