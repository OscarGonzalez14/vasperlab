...
$pacientes = new Reporteria();
$pacs = $pacientes->getPacientesOrder();?>
<div class="modal fade" role="dialog" id="nueva_orden_clinic">
...
<section style="margin:15px">
  ...
  <div class="form-row">
    <div class="form-group col-md-8 select2-purple">
      ...
      <select class="select2 form-control" id="empleado_clinic" multiple="multiple" data-placeholder="Seleccionar Paciente" data-dropdown-css-class="select2-purple" style="width: 100%;">             
      <option value="0">Seleccionar usuario</option><?php
        for ($i=0; $i < sizeof($pacs); $i++) { ?>
        <option value="<?php echo $pacs[$i]["id_paciente"]?>"><?php echo strtoupper($pacs[$i]["cod_emp"]." - ".$pacs[$i]["nombre"]." - ".$pacs[$i]["empresa"]);?></option>
        <?php  } ?>              
        </select>
    </div>
...
<button class="btn btn-primary btn-block" style="border-radius:0px" onClick="agregarOrdenClinic();"><i class="fas fa-save"></i> Guardar</button>
</section>
    </div>
  </div>
</div>
